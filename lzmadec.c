...
#define RC_READ_BYTE() do { \
  if (src == src_end) ERROR(); \
  a = *src++; \
}while(0)

#define RC_NORMALIZE() if (!(range >> 24)) { range <<= 8; RC_READ_BYTE(); code = code << 8 | a; }
...
int32_t decode_lzma(const uint8_t *src, size_t src_size, uint8_t *dst, size_t dst_size, uint16_t *probs) {
  const uint8_t *src_end = src + src_size;
  uint8_t *dst_start = dst, *dst_end = dst + dst_size;
  uint32_t dict_size = 0, num_probs; uint64_t max_size = 0;

  uint16_t /**probs = NULL,*/ *p1=probs, *p3=probs;
...
//  if (!probs) ERROR();
  ...
  while (max_size != (uint64_t)(dst - dst_start)) {
    ...
    if (bit0) {
      ...
      p3 = probs + 1846 + 768 * k;
      offset = i = 0;
      if (state >= 7) offset = 0x100, i = *(dst - rep0);
      len = value = 1;
      do {
        i <<= 1;
      ...
      *dst++ = value;
      ...
      if (!rep0) break;
      if (rep0 > dict_size) ERROR();
      if (rep0 > dst - dst_start) ERROR();
      if (max_size - (dst - dst_start) < len) ERROR();
      do {
        value = *(dst - rep0);
        if (dst == dst_end) ERROR();
        *dst++ = value;
